...
using a t statistic. Details of `BSCET` can be found in the Method section of [our manuscript](https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1009080).
...
Second, we calculate the subject-specific total number of cells in the bulk tissue, i.e., $m_j$. As $m_j$ is not observed for the bulk data, we esimate it using the individual-specific library size factor, which can be obtained by the function `estimateSizeFactor` of DESeq2 [@deseq2]. A detailed tutorial of DESeq2 [@deseq2] can be found on [this page](http://master.bioconductor.org/packages/release/workflows/vignettes/rnaseqGene/inst/doc/rnaseqGene.html).
